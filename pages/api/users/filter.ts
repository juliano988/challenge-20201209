...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const decodedUserLastNameRegex = new RegExp(decodeURI(req.query.userLastName as string), 'i');
  const userGenderRegex = new RegExp('^' + req.query.userGender as string, 'i');
  const requestedPage = Number(req.query.p);

  if (requestedPage) {
    const foundedUsers = await Users.find({ 'name.first': decodedUserNameRegex, 'name.last': decodedUserLastNameRegex, gender: userGenderRegex }).skip(50 * (requestedPage - 1)).limit(50).lean().exec() as Array<User>;
    if (foundedUsers.length) {
      const numberOfAllFoundedUsers = (await Users.find({ 'name.first': decodedUserNameRegex, 'name.last': decodedUserLastNameRegex, gender: userGenderRegex }).lean().exec()).length;
