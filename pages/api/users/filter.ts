...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (requestedPage) {
    const foundedUsers = await Users.find({ 'name.first': decodedUserNameRegex, gender: userGenderRegex }).skip(50 * (requestedPage - 1)).limit(50).lean().exec() as Array<User>;
    if (foundedUsers.length) {
      const numberOfAllFoundedUsers = (await Users.find({ 'name.first': decodedUserNameRegex, gender: userGenderRegex }).lean().exec()).length;
